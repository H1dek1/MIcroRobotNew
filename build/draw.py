...
def main():
    ...
    for step in tqdm(range(ext_x.size)):
        ...
        perm1 = patches.Circle(xy=(perm1_x[step], perm1_y[step]), radius=AbyL, fc='gray', ec='gray', fill=True, zorder=1)
        im_1_patch = axes[0].add_patch(perm1)
        im_1 = axes[0].quiver(perm1_x[step], perm1_y[step], perm1_u[step], perm1_v[step], color='black', angles='xy', scale_units='xy', scale=1, pivot='mid', width=4.0e-3, zorder=2)

        perm2 = patches.Circle(xy=(perm2_x[step], perm2_y[step]), radius=AbyL, fc='gray', ec='gray', fill=True, zorder=1)
        im_2_patch = axes[0].add_patch(perm2)
        im_2 = axes[0].quiver(perm2_x[step], perm2_y[step], perm2_u[step], perm2_v[step], color='black', angles='xy', scale_units='xy', scale=1, pivot='mid', width=4.0e-3, zorder=2)

        para = patches.Circle(xy=(para_x[step], para_y[step]), radius=AbyL, fc='orange', ec='orange', fill=True, zorder=1)
        im_3_patch = axes[0].add_patch(para)
        im_3 = axes[0].quiver(para_x[step], para_y[step], para_u[step], para_v[step], color='black', angles='xy', scale_units='xy', scale=1, pivot='mid', width=4.0e-3, zorder=2)
        ims.append(
