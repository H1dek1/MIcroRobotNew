...
def main():
    ...
    ax2_mini = fig.add_axes([0.05, 0.18, 0.3, 0.16])
    ax3_mini = fig.add_axes([0.53, 0.18, 0.3, 0.16])
    ...
    ax0.text(ext_x+1.6*np.cos(np.pi/12), ext_y+1.6*np.sin(np.pi/12), r'$\phi^{ext}$', fontsize=20, horizontalalignment='center', verticalalignment='center')
    ...
    ax0.annotate('',
            ...
            color='k',
            zorder=4)
    ax0.text(2.0*np.cos(phi/2), 2.0*np.sin(phi/2), r'$\phi^{v}$', fontsize=20, horizontalalignment='center', verticalalignment='center')
    ...
    ax1.set_ylabel(r'$\theta$', fontsize=25)
    #ax1.set_xlim(0, 2.5)
    ax1.set_ylim(-0.1, 1*np.pi/4)
    xticks = np.arange(0, 11.0, 2.0)
    xticklabels = [str(n) for n in xticks]
    ax1.set_xticks(xticks)
    ax1.set_xticklabels(xticklabels, fontsize=25)
    ax1.set_yticks([0, np.pi/8, np.pi/4])
    ax1.set_yticklabels(['0', '$\\pi/8$', '$\\pi/4$'], fontsize=25)
    ax1.plot(theta[:, 0], angle[:, 4], color="red", label=r'$\theta$')


    for ax in [ax2, ax3]:
        ax.set_xlabel(r'$\phi_B$', fontsize=25)
        ax.set_ylabel(r'$\phi_S$', fontsize=25)
    ...
    for ax in [ax2_mini, ax3_mini]:
        ...
        ax.set_ylabel(r'$y/l$', fontsize=15)
        ax.set_aspect('equal')
    ...
    ax2_mini.quiver(np.zeros(n), np.zeros(n), pos_x[::int(step)]/length[::int(step)], pos_y[::int(step)]/length[::int(step)], angles='xy', scale_units='xy', scale=1, color='C0')
    ...
    ax3_mini.quiver(np.zeros(n), np.zeros(n), pos_x[::int(step)]/length[::int(step)], pos_y[::int(step)]/length[::int(step)], angles='xy', scale_units='xy', scale=1,color='C0')
